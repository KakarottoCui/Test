...
public class YaopinController {
    ...
    @Autowired
    private YaopinCollectionService yaopinCollectionService;//猪饲料收藏
    @Autowired
    private YaopinCommentbackService yaopinCommentbackService;//猪饲料评价
    @Autowired
    private YaopinOrderService yaopinOrderService;//猪饲料订单
    @Autowired
    private YaopinrukuService yaopinrukuService;//猪饲料入库
    @Autowired
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            YaopinEntity yaopinEntity = new YaopinEntity();
//                            yaopinEntity.setYaopinName(data.get(0));                    //猪饲料名称 要改的
//                            yaopinEntity.setYaopinUuidNumber(data.get(0));                    //猪饲料编号 要改的
//                            yaopinEntity.setYaopinPhoto("");//详情和图片
//                            yaopinEntity.setChufangyaoTypes(Integer.valueOf(data.get(0)));   //配方饲料 要改的
//                            yaopinEntity.setYaopinTypes(Integer.valueOf(data.get(0)));   //猪饲料类型 要改的
//                            yaopinEntity.setYaopinErjiTypes(Integer.valueOf(data.get(0)));   //二级类型 要改的
//                            yaopinEntity.setYaopinKucunNumber(Integer.valueOf(data.get(0)));   //猪饲料库存 要改的
//                            yaopinEntity.setZanNumber(Integer.valueOf(data.get(0)));   //赞 要改的
//                            yaopinEntity.setCaiNumber(Integer.valueOf(data.get(0)));   //踩 要改的
//                            yaopinEntity.setYaopinOldMoney(data.get(0));                    //猪饲料原价 要改的
//                            yaopinEntity.setYaopinNewMoney(data.get(0));                    //现价 要改的
//                            yaopinEntity.setYaopinClicknum(Integer.valueOf(data.get(0)));   //猪饲料热度 要改的
...
//                            yaopinEntity.setCreateTime(date);//时间
                        ...
                        if(yaopinEntities_yaopinUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [猪饲料编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
